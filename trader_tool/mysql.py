...
class mysql:
    ...
    def insert_data_to_table_on(self,table_name='stock_data',title='open',value=1):
        ...
        try:
            ...
            print('插入成功')
            #查看结果
            self.show_table_context(table_name=table_name)
            self.db.close()
        except:
    ...
    def insert_data_excel_to_sql(self,table_name='excel',path=r'C:\Users\Administrator\Desktop\结果.xlsx'):
        ...
        for column in df.columns.tolist():
            self.add_table_columns_name(table_name=table_name,columns=column,endcode='varchar(225)')
        #删除id
        self.drop_table_columns_name(table_name=table_name,columns='id')
        #print(df)
    ...
    def adjiust_columns_postion(self,table_name='student',columns='open'):
        ...
        #先查看数据结果
        try:
            chart=self.desc_table(table_name=table_name)
