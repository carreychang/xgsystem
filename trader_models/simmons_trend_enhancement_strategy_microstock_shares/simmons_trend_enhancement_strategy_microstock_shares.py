...
import time
class simmons_trend_enhancement_strategy_microstock_shares:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                name='run_simmons_trend_enhancement_strategy_microstock_shares',
                data_api='qmt'):
        '''
        西蒙斯通达信微盘股精选趋势增强策略
        '''
        ...
        self.name=name
        self.data=unification_data(trader_tool=self.trader_tool,data_api=self.data_api)
        ...
        with open(r'{}\西蒙斯通达信微盘股精选趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\西蒙斯通达信微盘股精选趋势增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_trader_stock_data(self):
        ...
        df=self.get_read_tdx_data(path=r"{}\微盘股成分股\WPGJX.blk".format(self.path))
        df['证券代码']=df['证券代码'].apply(lambda x: '0'*(6-len(str(x)))+str(x))
        

        del_tuple = tuple(del_list)
        # 使用 startswith 方法判断前缀
        df['剔除'] = df['证券代码'].astype(str).apply(
            lambda x: '是' if x.startswith(del_tuple) else '不是'
        )
        df=df[df['剔除']=='不是']
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/西蒙斯通达信微盘股精选趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
