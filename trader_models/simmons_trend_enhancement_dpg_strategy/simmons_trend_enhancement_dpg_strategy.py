...
import time
class simmons_trend_enhancement_dpg_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                name='run_simmons_trend_enhancement_dpg_strategy',
                data_api='qmt'):
        '''
        西蒙斯大盘股指数趋势增强策略
        '''
        ...
        with open(r'{}\西蒙斯大盘股指数趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\西蒙斯大盘股指数趋势增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_trader_stock_data(self):
        ...
        df=self.get_read_tdx_data(path=r"{}\微盘股成分股\DPG.blk".format(self.path))
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/西蒙斯大盘股指数趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
