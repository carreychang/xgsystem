...
class global_major_asset_six_pulse_excalibur_rotation_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                name='run_global_major_asset_six_pulse_excalibur_rotation_strategy',
                data_api='qmt'):
        '''
        全球大类资产六脉神剑轮动策略
        '''
        ...
        with open(r'{}\全球大类资产六脉神剑轮动策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\全球大类资产六脉神剑轮动策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_buy_sell_stock_data(self):
        ...
        buy_df['交易状态']='未买'
        buy_df=buy_df.sort_values(by=['趋势得分','日周期'],ascending=False)
