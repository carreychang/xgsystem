...
class global_broad_class_low_correlation_trend_rotation_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                name='run_small_fruit_bonds_band_trading_strategy',
                data_api='qmt'):
        '''
        全球大类低相关性波段趋势轮动策略
        '''
        ...
        with open(r'{}\全球大类低相关性波段趋势轮动策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\全球大类低相关性波段趋势轮动策略.json'.format(self.path),'w',encoding='utf-8') as f:
