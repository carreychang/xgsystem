...
class simmons_convertible_bond_trading_system:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                name='run_simmons_convertible_bond_trading_system',
                data_api='qmt'):
        '''
        西蒙斯可转债自定义因子交易策略     
        '''
        ...
        self.name=name
        with open(r'{}/西蒙斯可转债自定义因子交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        self.url=text['服务器']
        self.port=text['端口']
        ...
        #保存
        with open(r'{}\西蒙斯可转债自定义因子交易策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_buy_sell_stock(self):
        ...
        获取买卖数据
        '''
        with open('{}/西蒙斯可转债自定义因子交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        buy_num=text['持有排名前N']
