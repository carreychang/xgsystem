...
class global_outer_disc_six_pulse_excalibur_trend_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                name='run_global_outer_disc_six_pulse_excalibur_trend_strategy',
                data_api='qmt'):
        '''
        全球外盘六脉神剑趋势策略
        '''
        ...
        with open(r'{}\全球外盘六脉神剑趋势策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\全球外盘六脉神剑趋势策略.json'.format(self.path),'w',encoding='utf-8') as f:
