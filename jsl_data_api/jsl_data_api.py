...
class jsl_data_api:
    ...
    获取集思录数据
    '''
    def __init__(self,user, password,limit=10):
    ...
    def get_all_connect_data(self):
        ...
        spot_df=df
        stock_list = df['转债代码'].tolist()[:self.limit]
        ...
        def process_stock(stock):
            nonlocal data
            try:
                df_part = self.get_convert_bond_detail(stock=stock)
                with lock:
                    data = pd.concat([data, df_part], ignore_index=True)
            except Exception:
                print(f'{stock}获取有问题')
        ...
        with ThreadPoolExecutor() as executor:
            # 使用tqdm显示进度
            list(tqdm(executor.map(process_stock, stock_list), total=len(stock_list)))
        ...
        for columns in data.columns.tolist():
            try:
                data[columns] = data[columns].apply(
                    lambda x: float(str(x).split('%')[0]) if '%' in str(x) else x)
            except:
                pass
        df=data
        spot_df=spot_df
...
if __name__=='__main__':
    models=jsl_data_api('18634899957','lw4022153',limit=10)
    models.login()
