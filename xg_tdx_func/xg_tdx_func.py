...
def SMA(S, N, M=1):   
    '''
    SMA,至少需要120周期才精确 (雪球180周期)    alpha=1/(1+com)    
    '''    
...
def CROSS_DOWN(S1, S2): 
    '''
    判断向上死叉穿越 CROSS(MA(C,5),MA(C,10))  判断向下死叉穿越 CROSS(MA(C,5),MA(C,10))   
    '''               
    return np.concatenate(([False], np.logical_not((S1<S2)[:-1]) & (S1<S2)[1:]))    # 不使用0级函数,移植方便  by jqz1226
